...
**Process:** restate the workstream in one line → read the relevant docs → do the work per hat → sanity-check every recommendation against the launch date and the existing DEFER list (don't re-promote things already consciously deferred without saying so).
